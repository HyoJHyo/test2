...
public class FileMove {
    ...
    * 파일을 p_thisPath 에서  p_thatPath 로 이동한다.
    @param thatPath 이동할 경로
    @param thisFile 이동하기전 경로
    @param fileName 파일명
    @return boolean  파일이동 성공여부
    */
    public boolean move(String thatPath, File thisFile, String fileName) {
        ...
        try {
            File tempFile =  thisFile; //new File(p_thisPath + system_slash + p_fileName);    //   임시폴더에서 이동할 파일명의 File 객체 생성한다
            ...
            File dir = new File(thatPath);
            if (!dir.isDirectory()) {       // 지정된 디렉토리가 존재여부 그리고 파일인지 체크
                dir.mkdirs();
            }

            File realFile = new File(thatPath + system_slash + fileName);    //   임시폴더에서 이동할 파일명의 File 객체 생성한다
